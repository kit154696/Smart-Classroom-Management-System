...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (histMatch && req.method==='GET') {
      let sql="SELECT a.attendance_date,a.check_in_time,a.status,c.course_id,c.course_name FROM Attendance a JOIN Course c ON a.course_id=c.course_id WHERE a.student_id=?";
      ...
      return json(res,200,{success:true,count:rows.length,data:rows.map(r=>({attendance_date:r.attendance_date,course_id:r.course_id,course_name:r.course_name,checkin_time:r.check_in_time,status:r.status}))});
    }
